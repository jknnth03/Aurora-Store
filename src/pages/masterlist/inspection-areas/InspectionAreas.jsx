...
import InspectionAreasModal from "./InspectionAreasModal";
import "./InspectionAreas.scss";
...
const InspectionAreas = () => {
  ...
  const { data, isFetching, error } = useGetInspectionAreasQuery({
    status: currentStatus,
    search: debouncedSearch,
    page,
    per_page: rowsPerPage,
  });
  const [archiveInspectionArea, { isLoading: isArchiving }] =
    useArchiveInspectionAreaMutation();
  ...
  const handleConfirmRestore = async () => {
    try {
      await archiveInspectionArea(toRestore.id).unwrap();
      window.__snackbar__?.enqueueSnackbar(
        "Inspection area restored successfully.",
        { variant: "success" },
      );
  ...
  const handleConfirmArchive = async () => {
    try {
      await archiveInspectionArea(toArchive.id).unwrap();
      window.__snackbar__?.enqueueSnackbar(
        "Inspection area archived successfully.",
        { variant: "success" },
      );
  ...
  return (
    <>
      <PageContainer
        ...
        titleAction={
          <UniversalButton
            label="Add Inspection Area"
            tooltip="Click this button to add a new inspection area"
            icon={<AddIcon />}
            onClick={handleAdd}
          />
        }
        actions={
          <>
            ...
            <TableSearchField
              ...
              placeholder="Search inspection areas..."
            />
          </>
        }
        pagination={
      ...
      <ConfirmDialog
        ...
        title="Archive Inspection Area"
        message={`Are you sure you want to archive "${toArchive?.name}"? This action will set the inspection area as inactive.`}
      />
